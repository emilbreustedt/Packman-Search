...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for legalAction in legalActions:
            if value < self.getQValue(state, legalAction):
                value = self.getQValue(state, legalAction)
    ...
    def computeActionFromQValues(self, state):
        ...
        legalActions = self.getLegalActions(state)
        for legalAction in legalActions:
            newValue = self.getQValue(state, legalAction)

            if value < newValue:
                value = newValue
                action = legalAction
            elif value == newValue:
                action = random.choice([action, legalAction])
